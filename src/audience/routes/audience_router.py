...
from src.audience.routes.port.usecase.csv_upload_usecase import CSVUploadUseCase
from src.audience.routes.port.usecase.delete_audience_usecase import (
    DeleteAudienceUseCase,
)
from src.audience.routes.port.usecase.download_audience_usecase import (
    DownloadAudienceUseCase,
)
from src.audience.routes.port.usecase.get_audience_creation_options_usecase import (
    GetAudienceCreationOptionsUseCase,
)
from src.audience.routes.port.usecase.get_audience_usecase import GetAudienceUseCase
from src.audience.routes.port.usecase.update_audience_exclude_status import (
...
from src.audience.service.background.execute_target_audience_summary import (
...
def get_audience_creation_options(
    ...
    audience = get_audience_service.get_audience_details(audience_id, db=db)
    ...
    if audience.create_type_code == AudienceCreateType.Filter.value:
        ...
        return get_audience_creation_option.get_csv_uploaded_data(audience_id, db=db)
...
def delete_audience(
    ...
    1. cust_campaign_objects
    """
    delete_audience_service.exec(audience_id=audience_id, db=db)
...
def download_audience(
    ...
    data = download_audience_service.exec(audience_id, db=db)
...
def get_csv_template(user=Depends(get_permission_checker(required_permissions=[]))):
    ...
    template_dir = pathlib.Path.cwd() / "src/audience/resources/message_template"
...
def audience_update_cycles(
    ...
    audience_update_cycle_service.exec(audience_id, cycle.value, db=db)
