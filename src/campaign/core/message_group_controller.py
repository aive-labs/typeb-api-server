...
class MessageGroupController:
    ...
    def pre_define_campaign_msg_seq(self):
        self.campaign_msg_gen_key = [
            ...
            for item 
            in self.set_group_msg_info if item.set_group_message.msg_send_type == "campaign" and item.set_group_message.msg_gen_key is not None
        ]
    ...
    def pre_define_remind_msg_seq(self, remind_step):
        self.remind_msg_gen_key = [
            item.set_group_message.msg_gen_key 
            for item 
            in self.set_group_msg_info if item.set_group_message.msg_send_type=="remind" and item.set_group_message.msg_gen_key is not None and item.set_group_message.remind_step==remind_step
        ]
    ...
    def get_set_group_seq(self, set_group_msg_seq):
        msg_seq_row = [
            item
            for item 
            in self.set_group_msg_info if item.set_group_message.set_group_msg_seq==set_group_msg_seq
        ][0]
    ...
    def get_msg_obj_from_seq(self, set_group_msg_seq):
        ...
        msg_obj = MessageGenerate(
            ...
            bottom_text=self.vender_bottom_txt[self.campaign_base.msg_delivery_vendor.value], #매장 번호 또는 대표번호
            msg_announcement="",
            template_id=None,
...
def message_image_validator(msg_obj: Message):
    ...
    if msg_obj.msg_type.value in ('mms', 'kakao_image_general', 'kakao_image_wide') and msg_obj.msg_photo_uri is None:
        raise HTTPException(
            status_code=400,
            detail={"code": "campaign/message", "message": f"{msg_type_dict[msg_obj.msg_type.value]}은(는) 이미지가 포함되어야 합니다."}
        )
    return msg_obj
...
def message_modify_validator(msg_delivery_vendor:str, msg_obj: Message):
    ...
    if  (msg_delivery_vendor == MsgDeliveryVendorEnum.DAU.value) and (encode_size > 40):
        raise HTTPException(
            status_code=400,
            detail={"code": "campaign/message/dau/title_size", "message": f"DAU 문자 메세지 제목은 40 Byte 이하로 입력해주세요. 입력한 Byte 크기: {encode_size}"}
        )

    if  (msg_delivery_vendor == MsgDeliveryVendorEnum.SSG.value) and (encode_size > 96):
        raise HTTPException(
            status_code=400,
            detail={"code": "campaign/message/ssg/title_size", "message": f"SSG 문자 메세지 제목은 96 Byte 이하로 입력해주세요. 입력한 Byte 크기: {encode_size}"}
    ...
    if len(msg_obj.msg_body) > 1000:
        raise HTTPException(
            status_code=400,
            detail={"code": "campaign/message", "message": "본문은 1000자 이하로 입력해주세요."}
        )
    ...
    if msg_obj.media.value=="tms":
        ...
        if msg_obj.msg_type.value=="kakao_image_general":
            if len(msg_obj.msg_body) > 400:
                raise HTTPException(
                    status_code=400,
                    detail={"code": "campaign/message", "message": "카카오 이미지 일반형은 400자 이하이어야 합니다."}
                )
        if msg_obj.msg_type.value=="kakao_text":
            if len(msg_obj.msg_body) > 1000:
                raise HTTPException(
                    status_code=400,
                    detail={"code": "campaign/message", "message": "카카오 텍스트형은 1000자 이하이어야 합니다."}
                )
        if msg_obj.msg_type.value=="kakao_image_wide":
            if len(msg_obj.msg_body) > 76:
                raise HTTPException(
                    status_code=400,
                    detail={"code": "campaign/message", "message": "카카오 이미지 와이드형은 76자 이하이어야 합니다."}
                )
...
def message_validator(msg_delivery_vendor:str, msg_obj: Message):
    ...
    # 메시지 타입별 validation checker
    if msg_obj.media.value=="tms":
        if (msg_obj.msg_type == MessageTypeEnum.MMS.value) & (msg_obj.msg_photo_uri is None):
            raise HTTPException(
                status_code=400,
                detail={"code": "campaign/message", "message": "MMS는 이미지가 포함되어야 합니다."}
            )

        if (msg_obj.msg_type == MessageTypeEnum.SMS.value) & (len(msg_obj.msg_body + msg_obj.bottom_text) > 45):
            raise HTTPException(
                status_code=400,
