...
class GenerateContentsService(GenerateContentsUseCase):
    ...
    def _product_resource_handler(
        ...
        youtube_resource = [
            ...
            if entity.link_type == 'youtube' if entity.link
        ]

        instagram_resource = [
            entity.link
            for entity in product_media_entities
            if entity.link_type == 'instagram' if entity.link
        ]
        ...
        review_list = [
            ...
            for item
            in review_entities if item.content
        ][:max_k]

        if len(review_list) > 0:
            review_resource = {
                'title': "실제 구매하신 분들의 생생한 리뷰",
                'reviews': review_list
            }
        else:
            review_resource = {}
    ...
    async def _prepare_contents_resource(self, contents_generate: ContentsGenerate, db):
        ...
        contents_menu: ContentsMenu = self.contents_repository.get_subject_by_code(contents_generate.subject, db)
        query = ""
        kwargs_dict = {
        ...
        try:
            if contents_generate.subject:
                ...
                # ]
                product_list = [item.product_code for item in contents_generate.product_object] if len(
                    contents_generate.product_object) > 0 else []
            ...
            if contents_generate.product_object:
                ...
                count_of_rep_nm = len(set([product.rep_nm for product in product_data]))
                ...
                if count_of_rep_nm == 1:
                    ...
                    kwargs_dict['summary'] = ''
                # Additional data
                product_resource_entity = self.contents_repository.get_product_media_resource(product_code_list, db)
    ...
    async def exec(self, contents_generate: ContentsGenerate, db):
        ...
        selected_template, query, kwargs_dict = await self._prepare_contents_resource(contents_generate, db)
        # return selected_template, query
        async for i in self.chain.generate_response(selected_template, query, **kwargs_dict):
            yield i
        # yield self.chain.generate_response(selected_template, query, **kwargs_dict)
        # yield self.chain.generate_response(selected_template, query, **kwargs_dict)
        # return result
